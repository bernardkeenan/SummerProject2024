document.getElementById('backButton').addEventListener('click', () => {
    window.location.href = 'chips.html'; // Redirect to menu.html
});
...
function startNewQuestion() {
    ...
    timer = setInterval(() => {
        ...
        if (timeRemaining <= 0) {
            ...
            document.getElementById("feedback").textContent = `Time's up! The correct Effective M-score was ${Effective_M}.`;
    ...
    document.getElementById("checkGuess").onclick = function () {
        ...
        if (user_guess === Effective_M) {
            feedbackElement.textContent = "Correct! Your guess is right.";
            feedbackElement.style.color = "green";
            correctAnswers++;
        } else {
            feedbackElement.textContent = `Wrong! The correct Effective M-score is ${Effective_M}.`;
